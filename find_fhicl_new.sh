...
if [ x$FHICL_FILE_PATH = x ]; then
  ...
  echo "Try 'setup dunetpc vXX_XX_XX -q eXX:debug"
  echo "OR 'mrbsetenv'"
  echo "***************************************"
  exit 1
...
if [ x$FHICL_SEARCH_FILE = x ]; then
  ...
  echo "Try 'setup dunetpc vXX_XX_XX -q eXX:prof"
...
SEARCH_PATHS=(`awk '{split($0,array,":"); for (a in array)  printf "%s ", arr\
ay[a]; printf "\n";}' <<< $FHICL_FILE_PATH`)

if [ ! -d "srcs" ]; then
  echo "***************************************"
  echo "NOTE: I do not see a 'srcs' directory in PWD ($PWD)"
  echo "I will continue to search \$FHICL_FILE_PATH,"
  echo "but you will unlikely have write access to the file."
  echo "Check out a version of dunetpc using the instructions"
  echo "Provided here: "
  echo "https://cdcvs.fnal.gov/redmine/projects/dunetpc/wiki/"
  echo "***************************************"
